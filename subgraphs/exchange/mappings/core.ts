...
import { updatePairDayData, updateTokenDayData, updateBigSwapDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  bigswap.totalLiquidityBNB = bigswap.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  bigswap.totalLiquidityBNB = bigswap.totalLiquidityBNB.plus(trackedLiquidityBNB);
  bigswap.totalLiquidityUSD = bigswap.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  bigswap.totalTransactions = bigswap.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let bigswap = BigSwapFactory.load(FACTORY_ADDRESS);
  bigswap.totalVolumeUSD = bigswap.totalVolumeUSD.plus(trackedAmountUSD);
  bigswap.totalVolumeBNB = bigswap.totalVolumeBNB.plus(trackedAmountBNB);
  bigswap.untrackedVolumeUSD = bigswap.untrackedVolumeUSD.plus(derivedAmountUSD);
  bigswap.totalTransactions = bigswap.totalTransactions.plus(ONE_BI);
  ...
  let bigswapDayData = updateBigSwapDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  bigswapDayData.dailyVolumeUSD = bigswapDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  bigswapDayData.dailyVolumeBNB = bigswapDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  bigswapDayData.dailyVolumeUntracked = bigswapDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  bigswapDayData.save();
